...
| `coupling` | Holds two halves rigidly under thrust until fired, then throws them apart rather than merely letting go; stays released; leaves each half under its own power; reports to a program whether it has gone; and is reported as seized when the build welds across it. How hard it throws is set per coupling: a higher setting measurably throws harder, zero lets go and adds nothing so you can push it yourself, and a setting outside the range is pulled back to the nearest end. |
